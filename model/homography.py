...
def perspective(cam_coords, proj_mat, h, w, extrinsic, offset=None):
    ...
    """
    eps = 1e-7
    pix_coords = proj_mat @ cam_coords#24 4 20000 = 24 4 4 * 4 20000
    ...
    if extrinsic:
        ...
        # pix_coords[:, 0] = pix_coords[:, 0] / pix_coords[:, 1]
        # pix_coords[:, 2] = pix_coords[:, 2] / pix_coords[:, 1]
...
def bilinear_sampler(imgs, pix_coords):
    ...
    y_max = (img_h - 1)#32
    x_max = (img_w - 1)#88
    pix_x = torch.clip(pix_x, 0, x_max)  # z,
    pix_y = torch.clip(pix_y, 0, y_max)  # x,

    pix_x0 = torch.clip(pix_x0, 0, x_max)#z,
    pix_y0 = torch.clip(pix_y0, 0, y_max)#x,
    pix_x1 = torch.clip(pix_x1, 0, x_max)#x_max
    pix_y1 = torch.clip(pix_y1, 0, y_max)#0, y_max
...
def plane_grid(xbound, ybound, zs, yaws, rolls, pitchs, h_w, cuda=True):
    B = len(zs)

    xmin, xmax = xbound[0], xbound[1]#-30 30
...
def ipm_from_parameters(image, xyz, K, RT, target_h, target_w, extrinsic, post_RT=None):
    ...
    if post_RT is not None:
        P = post_RT @ P
    P = P.reshape(-1, 4, 4)#24 4 4
    pixel_coords = perspective(xyz, P, target_h, target_w, extrinsic, image.shape[1:3])#转为相机坐标系
...
class IPM(nn.Module):
    def __init__(self, xbound, ybound, N, C, z_roll_pitch=False, visual=False, extrinsic=False, cuda=True):
        ...
        self.conv = nn.Conv2d(128, 64, kernel_size=1, padding=0)
        if z_roll_pitch:
            ...
            self.planes_1 = plane_grid(self.xbound, self.ybound, zs, yaws, rolls, pitchs,0.5)[0]
            #self.planes_2 = plane_grid(self.xbound, self.ybound, zs, yaws, rolls, pitchs, 1.3)[0]
        ...
        # self.STN_n = []
        # for i in range(6):
        #     self.STN_n.append(STNNet())
        # self.STN_n = nn.ModuleList(self.STN_n)
    ...
    def STN_enhance(self,x):
        #b,n,h,w,c = x.shape
        x = x.permute(0, 1, 4, 2, 3).contiguous()
        outputs = []
        for i in range(6):
            x_i = self.STN_n[i](x[:,i,:,:,:])
            outputs.append(x_i)
    ...
    def forward(self, images, Ks, RTs, translation, yaw_roll_pitch, post_RTs=None):
        ...
        if self.z_roll_pitch:
            ...
            planes_1 = self.planes_1#4 20000
            #planes_2 = self.planes_2  # 4 20000

        images = images.reshape(B*N, H, W, C)
        warped_fv_images_1 = ipm_from_parameters(images, planes_1, Ks, RTs, self.h, self.w, self.extrinsic, post_RTs)
        # warped_fv_images_2 = ipm_from_parameters(images, planes_2, Ks, RTs, self.h, self.w, self.extrinsic, post_RTs)
        # warped_fv_images = torch.cat((warped_fv_images_1,warped_fv_images_2),dim=-1)#24 100 200 128
        # warped_fv_images = warped_fv_images.permute(0,3,1,2).contiguous()
        # warped_fv_images = self.conv(warped_fv_images)#24 64 100 200
        # warped_fv_images = warped_fv_images.permute(0, 2, 3, 1).contiguous()
        warped_fv_images = warped_fv_images_1.reshape((B, N, self.h, self.w, C))
        if self.visual:
        ...
        if self.visual:
            ...
            #warped_fv_images = self.STN_enhance(warped_fv_images)#4 6 64 100 200

            warped_topdown, _ = warped_fv_images.max(1)
            #warped_topdown = torch.sum(warped_fv_images,dim=1)#多视角相加
