...
class CamEncode(nn.Module):
    ...
    def get_depth_feat(self, x):
        x = self.get_eff_depth(x)# 20 512 8 22
        # Depth#
        x = self.depthnet(x)# 20 105 8 22

        depth = self.get_depth_dist(x[:, :self.D])#20 41 8 22 学习深度预测

        new_x = depth.unsqueeze(1) * x[:, self.D:(self.D + self.C)].unsqueeze(2)#选取64维图像特征 * 深度预测
...
class LiftSplat(nn.Module):
    def __init__(self, grid_conf,  instance_seg, embedded_dim):#, data_aug_conf
        ...
        self.bevencode = BevEncode(inC=self.camC, outC=grid_conf['num_channels'])
    ...
    def forward(self,  x, trans, rots, intrins, post_trans, post_rots, translation, yaw_pitch_roll):
